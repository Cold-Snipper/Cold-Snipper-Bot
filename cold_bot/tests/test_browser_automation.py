...
def test_scroll():
    if not os.getenv("RUN_FUNCTIONAL"):
        return
    p, browser, context, page = browser_automation.init_browser(headless=False)
    browser_automation.scroll_and_navigate(page, "https://www.google.com", 5, 1, 2)
    page.screenshot(path="test_scroll.png")
    browser_automation.close_browser(p, browser, context)
...
def test_proxy():
    with patch("silos.browser_automation.sync_playwright") as mock_playwright:
        ...
        p, browser, context, page = browser_automation.init_browser(
...
def test_delays():
    with patch("utils.time.sleep") as mock_sleep, patch(
        "silos.browser_automation.sync_playwright"
    ) as mock_playwright:
        mock_p = MagicMock()
        mock_browser = MagicMock()
        mock_context = MagicMock()
        ...
        p, browser, context, page = browser_automation.init_browser(headless=True)
...
def test_errors():
    with patch("silos.browser_automation.sync_playwright") as mock_playwright:
        ...
        # First start() raises; second (after retry) returns mock_p
